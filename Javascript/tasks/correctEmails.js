function correctEmails(pattern) {
  ...
  var isCorrectEmail = function(emailList) {
    ...
    return count(email, '@') === 1 && count(email, '.') >= 1 && email.lastIndexOf('.') > email.lastIndexOf('@') && email.indexOf("..") === -1 && email.indexOf(".@") === -1 && email.indexOf("@.") === -1 && isSymbol(email.charAt(0)) && isSymbol(email.charAt(email.length - 1));
  };
